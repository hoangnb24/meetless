...
describe("M5 real-Codex composition proof validation", () => {
  ...
  test.each([
    ...
    ["supported-retry", (value: any) => { value.observation.chat.supported.operationalRetryCount = 3; }],
    ...
    ["workspace-cleanup", (value: any) => { value.observation.chat.restart.workspaceAbsentAfterRestart = false; }],
    ["insufficient-evidence", (value: any) => { value.observation.chat.unsupported.text = "invented"; }],
    ["unsupported-retry", (value: any) => { delete value.observation.chat.unsupported.operationalRetryCount; }],
  ...
  return {
    ...
    observation: {
      ...
      chat: {
        ...
        supported: { outcome: "supported", text: "The third interval.", citationSegmentIds: ["segment-2"], operationalRetryCount: 0 },
        citationPlayback: { boundedStopObserved: true, markerHz: 880, markerPowerRatio: 8 },
        restart: { exactInstalledHost: true, historyRestored: true, workspaceAbsentAfterRestart: true },
        unsupported: { outcome: "insufficient_evidence", text: null, citationSegmentIds: [], canonicalRendered: true, operationalRetryCount: 0 },
