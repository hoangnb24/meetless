...
| Packaged local signed app path | `scripts/verify_recordit_release_context.sh`; `scripts/gate_packaged_live_smoke.sh`; `scripts/gate_dmg_install_open.sh`; `README.md` packaged commands | shell | `packaged-app` + `fixture` | signed `dist/Recordit.app` payload/signing checks, packaged smoke artifact generation, and retained DMG install/open verification | packaged live smoke still uses deterministic fake capture and compatibility runtime assistance | onboarding + first live start/stop are still not covered in the installed-DMG lane | `bd-78qy`, `bd-13tm` |
...
| DMG install / mount / drag-to-Applications / open | `scripts/gate_dmg_install_open.sh`; `scripts/create_recordit_dmg.sh`; `README.md` local DMG instructions | shell | `packaged-app` | retained automated verifier mounts DMG, validates layout, copies app, launches installed app, and records contract-validated diagnostics | lane currently stops at install/open and does not include onboarding or first live stop | install-surface proof upgraded from docs-only to retained scripted evidence | `bd-78qy`, `bd-13pv` |
