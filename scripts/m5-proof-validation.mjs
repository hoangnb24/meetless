...
export function validateM5Observation(input) {
  ...
  requireValue(Number.isInteger(chat.supported?.operationalRetryCount) && chat.supported.operationalRetryCount >= 0 && chat.supported.operationalRetryCount <= 2, "supported-retry", "the supported turn has no bounded retry count", "record zero to two operational retries for the supported question");
  ...
  requireValue(Number.isInteger(chat.unsupported?.operationalRetryCount) && chat.unsupported.operationalRetryCount >= 0 && chat.unsupported.operationalRetryCount <= 2, "unsupported-retry", "the unsupported turn has no bounded retry count", "record zero to two operational retries for the unsupported question");
